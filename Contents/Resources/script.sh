...
# 清屏并隐藏之前的命令
clear
printf '\033]0;Install APK\007'  # 设置窗口标题
...
check_for_updates() {
    ...
    # 检查是否需要检查更新（每天检查一次，除非强制检查）
    if [ "$force_check" != "true" ] && [ -f "$LAST_UPDATE_CHECK_FILE" ]; then
        local last_check=$(cat "$LAST_UPDATE_CHECK_FILE")
    ...
    echo "${tty_cyan}正在检查更新...${tty_reset}"
    ...
    latest_release_info=$(curl -s --max-time 10 "$UPDATE_CHECK_URL" 2>/dev/null)
    
    if [ $? -ne 0 ] || [ -z "$latest_release_info" ]; then
        if [ "$force_check" == "true" ]; then
            echo "${tty_yellow}无法获取版本信息，请检查网络连接${tty_reset}"
        fi
        return 1
    fi
    ...
    latest_version=$(echo "$latest_release_info" | grep '"tag_name"' | sed 's/.*"tag_name": "v*\([^"]*\)".*/\1/')
    
    if [ -z "$latest_version" ]; then
        if [ "$force_check" == "true" ]; then
            echo "${tty_yellow}无法解析版本信息${tty_reset}"
        fi
        return 1
    fi
    ...
    # 检查是否已跳过此版本
    if [ -f "$CONFIG_DIR/skipped_version" ]; then
        local skipped_version=$(cat "$CONFIG_DIR/skipped_version")
        if [ "$latest_version" = "$skipped_version" ] && [ "$force_check" != "true" ]; then
            return 0  # 静默跳过已跳过的版本
        fi
    fi
    ...
    if version_gt "$latest_version" "$CURRENT_VERSION"; then
        # 获取下载链接
        local download_url
        download_url=$(echo "$latest_release_info" | grep '"browser_download_url"' | grep '\.dmg"' | head -1 | sed 's/.*"browser_download_url": "\([^"]*\)".*/\1/')
        ...
        if [ -n "$download_url" ]; then
            ...
            if [ "$force_check" == "true" ]; then
                echo "${tty_yellow}找到新版本 $latest_version，但无法获取下载链接${tty_reset}"
            fi
            return 1
        fi
    else
        if [ "$force_check" == "true" ]; then
            echo "${tty_green}当前已是最新版本 ($CURRENT_VERSION)${tty_reset}"
        fi
        return 0
    fi
}
...
show_update_dialog() {
    ...
    echo ""
    echo "${tty_bold}${tty_green}🚀 发现新版本！${tty_reset}"
    echo ""
    echo "${tty_green}当前版本: $CURRENT_VERSION${tty_reset}"
    echo "${tty_green}最新版本: $new_version${tty_reset}"
    echo ""
    echo "${tty_green}正在自动下载更新...${tty_reset}"
    
    download_and_install_update "$new_version" "$download_url"
}
...
download_and_install_update() {
    local new_version="$1"
    local download_url="$2"
    local download_file="$TEMP_DIR/Install_APK_v${new_version}.dmg"
    ...
    # 下载文件
    if curl -L -o "$download_file" "$download_url" --silent --show-error 2>/dev/null; then
        # 停止进度条动画
        ...
        if [ -f "$download_file" ] && [ -s "$download_file" ]; then
            install_update "$download_file" "$new_version"
        else
            echo "${tty_red}❌ 下载的文件无效${tty_reset}"
            return 1
        fi
    else
        # 停止进度条动画
...
install_update() {
    local dmg_file="$1"
    local new_version="$2"
    local mount_point="/Volumes/Install APK v${new_version}"
    
    echo "${tty_green}正在安装更新...${tty_reset}"
    
    # 挂载DMG
    echo "${tty_green}正在挂载安装文件...${tty_reset}"
    if hdiutil attach "$dmg_file" -quiet 2>/dev/null; then
        
        # 查找挂载点
        local actual_mount_point
        # 简化挂载点查找，避免显示错误信息
        for mp in "/Volumes/Install APK"*; do
            if [ -d "$mp" ]; then
                actual_mount_point="$mp"
                break
            fi
        done
        
        if [ -n "$actual_mount_point" ] && [ -d "$actual_mount_point" ]; then
            echo "${tty_green}正在复制应用程序...${tty_reset}"
            
            # 获取当前应用程序路径
            local current_app_path
            current_app_path=$(dirname "$0")
            current_app_path=$(dirname "$current_app_path")
            current_app_path=$(dirname "$current_app_path")
            ...
            if cp -R "$actual_mount_point/Install APK.app/Contents"/* "$current_app_path/Contents/" 2>/dev/null; then
                ...
                # 卸载DMG
                hdiutil detach "$actual_mount_point" -quiet 2>/dev/null
                ...
                echo "${tty_red}❌ 复制文件失败${tty_reset}"
                # 恢复备份
                if [ -d "$backup_path" ]; then
                    rm -rf "$current_app_path" 2>/dev/null
                    mv "$backup_path" "$current_app_path" 2>/dev/null
                    echo "${tty_yellow}已恢复原版本${tty_reset}"
                fi
                hdiutil detach "$actual_mount_point" -quiet 2>/dev/null
                return 1
            fi
        else
            echo "${tty_red}❌ 无法找到挂载点${tty_reset}"
            return 1
        fi
    else
        echo "${tty_red}❌ 无法挂载安装文件${tty_reset}"
        return 1
    fi
}
...
check_and_install_dependencies() {
    ...
    if [ "$all_ready" = true ]; then
        echo "${tty_green}✅ 环境检查完成 - 所有依赖已就绪${tty_reset}"
        return 0
    fi
...
show_dynamic_progress() {
    ...
    # 显示绿色进度条（与下载进度条样式一致）
    printf "\r${tty_green}安装进度: "
    for ((j=0; j<completed; j++)); do printf "█"; done
    for ((j=completed; j<width; j++)); do printf "░"; done
    printf " %d%%${tty_reset}" "$percent"
}
